            ...
            <div class="col-lg-8 offset-lg-2 col-sm-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('populationStore') }}" method="post">
                            @csrf
                            <div class="form-group">
                                <label>Card number</label>
                                <input type="text" name="card_number" class="form-control {{ $errors->has('name') ? "is-invalid":"" }}" value="{{ old('card_number') }}" required>
                            ...
                            <div class="form-group">
                                <label>Owner</label>
                                <select class="form-select" aria-label="Default select example" name="client_id" value="{{ old('client_id') }}">
                                    @foreach($clients as $client)
                                        <option value={{ $client['id'] }}>{{ $client['name'] .' ' .$client['sure_name'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
